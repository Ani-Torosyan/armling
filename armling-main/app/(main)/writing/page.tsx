  ...
  useEffect(() => {
    const fetchExercise = async () => {
      try {
        ...
        setExercise(response.data.WritingExercises[0]); // Assuming response.data is an array
        setLoading(false);
      } catch (error) {
            ...
            <div key={index}>
              ...
              <div>
                {sentence.words.map((word, i) => (
                  <Button key={i} onClick={() => setUserAnswer((prev) => [...(prev || []), word])}>
                    {word}
                  </Button>
                ))}
              </div>
              <p>Correct Answer: {sentence.correctAnswer}</p>
            </div>
          ))}
        </div>
      </FeedWrapper>
